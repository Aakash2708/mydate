...
class Payroll:
    ...
    def fetch_all(self):
        ...
        cur = self.con.cursor()
        cur.execute(query)
        print(tabulate(cur, headers=['Email', 'job', 'BasicSalary', 'DA', 'HRA', 'GrossSalary', 'Tax', 'NetSalary', 'Month', 'Year', 'PAID_UNPAID'],tablefmt='psql'))
    ...
    def updaterec_pay(self, email,  job,  BasicSalary,  DA,  HRA,  GrossSalary,  Tax,  NetSalary, PAID_UNPAID):
        query = "  update pay set job='{}',BasicSalary='{}',DA='{}',HRA='{}',GrossSalary='{}',Tax='{}',NetSalary='{}',PAID_UNPAID='{}'where email='{}'".format(job, BasicSalary, DA, HRA, GrossSalary, Tax, NetSalary,PAID_UNPAID,email)
        cur = self.con.cursor()
        cur.execute(query)
        self.con.commit()
        print("updated")
...
while True:
    ...
    if choice == 1:
        try:
            ...
            NetSalary = GrossSalery - BasicSalery
        ...
        try:
            ...
            msg = EmailMessage()
            msg.set_content("Hi\n" + str(username) + " your salary is created to your account  for the month of " + str(Month) + " \nFor further details contact to your HR.\n\nThank You\nVirtual office team;")
            msg['Subject'] = 'salery created'
            msg['From'] = input("Enter your email address  to send mail to Employee")
            msg['To'] = email
            password=input("enter the password of email to send mail")
